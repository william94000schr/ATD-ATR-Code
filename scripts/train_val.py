...
def main(num_classes, num_epochs, proportion):
    ...
    train_dataset, val_dataset = random_split(
        full_dataset, [train_size, val_size], generator=generator1
    )
    ...
    for epoch in range(num_epochs):
        ...
        results.append(
            {"epoch": epoch, "learning_rate": current_lr, "train": train_result, "val": val_result}
        )
    ...
    print(f"Model saved to: {project_root}/experiments/models/faster_rcnn.pt")
    print(f"Results saved to: {project_root}/experiments/outputs/train_results.json")
